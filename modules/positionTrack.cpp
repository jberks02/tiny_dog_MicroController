...
//(0, 125) default xy plane position
//(125, 0) default yz plane position
...
class ExtensionTracker {
    ...
    private: int xy[2];
    //in order of x,y,z
    private: int coordinate[3];
    private: float ServoAngles[3];
    private: char servoMovements[3];
    public: ExtensionTracker(int yzPlane[2],int xyPlane[2], int defaultCoordinate[3], float defaultServoAngles[3], char servoMovementTypes[3]) {
        memcpy(yz, yzPlane, 2);
        memcpy(xy, xyPlane, 2);
    ...
    private: float convertCAngleOnNormalTriangle(float armAngle) {
